    ...
    <div class="relative z-10 w-full max-w-md p-6 lg:p-8 mx-4 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)] animate-fade-in delay-100 lg:ml-20">
        ...
        <div class="mb-6 text-center">
            <div class="inline-flex items-center gap-3 mb-2">
                <i class='bx bx-desktop text-4xl text-red-500'></i>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="animate-fade-in delay-300">
                ...
                <div class="relative">
                    ...
                    <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center">
                        <button type="button" id="togglePassword" class="text-gray-500 hover:text-red-500 focus:outline-none transition-colors">
            ...
            <div class="flex items-center justify-between pt-1 animate-fade-in delay-400">
                <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                    <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 text-red-600 bg-black/30 border-white/10 rounded focus:ring-red-500 cursor-pointer transition-all">
                    <span class="ms-2 text-sm text-gray-300 font-medium group-hover:text-red-500 transition-colors">Ingat saya</span>
                </label>

                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm font-bold text-red-400 hover:text-red-300 transition-colors">
                        Lupa password?
                    </a>
                @endif
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            togglePassword.addEventListener('click', function (e) {
                ...
                if(type === 'text') {
                    ...
                    eyeIcon.classList.add('bx-show', 'text-red-400');
                } else {
                    eyeIcon.classList.remove('bx-show', 'text-red-400');
